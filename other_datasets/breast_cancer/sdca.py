...
class SDCA:
    ...
    x is a list of points with norm <= 1 (transposed vectors), a_0 is a transposed vector
    lamb is a lambda regularization parameter, recommended to be 0.00001"""
    ...
    def predict(self, X):
        values = X.dot(self.w.T)
        y = [(0 if val < 0 else 1) for val in values]
        return np.array(y)

    def compute_updates(self, x, y, a, epochs, lamb):
        # calculate initial omega from initial alpha
        w_0 = np.array(sum([a[i]*x[i] for i in range(len(x))])/(lamb*len(x)))
        ...
        perm = np.array([np.arange(len(x))]).T
    ...
    def compute_update(self, i, x, y, a, w, lamb):
        ...
        a_grad = self.compute_alpha_gradient(a[i], x[i], y[i], w, lamb, n)

        # use gradient to adjust the alpha and omega values
        new_a[i] += a_grad
        new_w += a_grad * x[i] / (lamb * n)
        return (new_a, new_w)
    ...
    def compute_alpha_gradient_log(self, a, x, y, w, lamb, n):
        inside_term = 1.0 + np.exp(x.dot(w.T) * y)
        numerator = y / inside_term - a
        denominator = max(1.0, 0.25 + (np.linalg.norm(x) ** 2)/(lamb * n))
    ...
    def compute_alpha_gradient_smooth_hinge(self, a, x, y, w, lamb, n, gamma=1.0):
        numerator = 1.0 - y * x.dot(w.T) - gamma * a * y
        denominator = (np.linalg.norm(x)**2)/(lamb * n) + gamma #lamb*n in the denom
    ...
    def compute_alpha_gradient_hinge(self, a, x, y, w, lamb, n):
        numerator = 1.0 - y * x.dot(w.T)
        denominator = (np.linalg.norm(x)**2)/(lamb*n)
        value = numerator/denominator + a*y
        return y * max(0.0, min(1.0, value)) - a
